...
class ActiveDirectory {
    ...
    async loginUser(username, password, customBase, customSearch) {
        try {
            ...
            const searchUser = usernameType === 'sAMAccountName' ? username.split('\\')[1].split('@')[0] : username;
            const search = {
                ...this.searchOptions,
     ...
     * @param {string} password The users password
     * @param {string=} customBase Override the default class base, if not passed the class base is used.
     * @param {booleans} detailed Indicates if you want the detailed groups objects with name, dn, guid, description, created, and changed values
